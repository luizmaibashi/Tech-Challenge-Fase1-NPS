...
aqui (config.DGP_*). O prototipo serve para mostrar que a analise recupera o que foi
plantado e aplica a regra de decisao economica; nao afirma nada sobre o cupom real.
...
@dataclass
class CenarioDGP:
    efeito: str = "heterogeneo"          # chave de cfg.DGP_EFEITO_POR_ESTRATO
    meses: int = cfg.DGP_MESES_PADRAO
    ...
    contaminacao_controle: float = cfg.DGP_CONTAMINACAO_CONTROLE
    ancora_janela: str = cfg.DGP_ANCORA_JANELA   # "entrega" | "acao"
    seed: int = cfg.SEED
    _efeitos: list = field(default_factory=list, repr=False)

    def efeito_por_estrato(self):
        return cfg.DGP_EFEITO_POR_ESTRATO[self.efeito]
...
def gerar_populacao(cenario: CenarioDGP | None = None) -> pd.DataFrame:
    ...
    efeitos = cen.efeito_por_estrato()
    ...
    p0_base = np.where(det, np.array(cfg.DGP_P0_POR_ESTRATO)[est], cfg.DGP_P0_NAO_DETRATOR)
    ...
    efeito_nd = cfg.DGP_EFEITO_NAO_DETRATOR if any(efeitos) else 0.0
    ...
    # tempos (dias a partir da data da entrega com atraso = ancora correta)
    df["dias_ate_acao"] = cfg.ATRASO_ENVIO_ACAO_DIAS + rng.integers(0, 2, len(df))
    resp = rng.geometric(p=1 / 8, size=len(df))          # ~8 dias em media
    respondeu = rng.uniform(0, 1, len(df)) < 0.35        # so ~35% respondem a pesquisa
    df["dias_ate_resposta"] = np.where(respondeu, resp, -1)
...
if __name__ == "__main__":
    for chave in ("heterogeneo", "nulo", "rentavel"):
        pop = gerar_populacao(CenarioDGP(efeito=chave, meses=cfg.DGP_MESES_PADRAO))
